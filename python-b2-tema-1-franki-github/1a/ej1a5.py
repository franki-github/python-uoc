...
def add_product(name: str, category: str, quantity: int, price: float) -> Product:
    ...
    print(inventory[key], "/n")    
    return inventory[key].name, inventory[key].category, inventory[key].quantity, inventory[key].price



def list_products() -> str:
    # Complete the code
    print("...en list_products....")
    if not inventory:
        return "No products in inventory."

    
    for product in inventory.values():
        print(f"{product.name} ({product.category}) - {product.quantity} units at ${product.price} each")
    #return
