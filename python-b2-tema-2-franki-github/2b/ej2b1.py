...
def read_csv_basic(file_path: str) -> pd.DataFrame:
    ...
    # Aseguramos de que el archivo existe y es accesible en la ruta que recibmos como parámetro(file_path)

    if not Path(file_path).exists():
    # Si el archivo no existe, lanzamos una excepción
        raise FileNotFoundError(f"El archivo {file_path} no existe.")
    
    # Leer un archivo CSV básico y devolver un DataFrame de Pandas
...
def read_csv_header_issue(file_path: str, header_row: int) -> pd.DataFrame:
    ...
    # Leer un archivo CSV donde los datos comienzan después de la fila especificada
...
def read_csv_multi_index(file_path: str, index_cols: t.List[str]) -> pd.DataFrame:
    # Write here your code
    # Leer un archivo CSV y utilizar múltiples columnas como índices
...
def read_csv_custom_separator(
    file_path: str, separator: str, decimal: str) -> pd.DataFrame:
    # Write here your 
    # Leer un archivo CSV utilizando un separador diferente al predeterminado y convertir la columna 'Stars' a tipo flotante
...
# Mostrar los primeros registros de cada DataFrame
print(
  "___ BASIC HEAD___________________________________________________________________________\n",   
    df_basic.head(),
    "__HEADER ISSUE ________________________________________________________________________\n",
    df_header_issue.head(),
     "_ MULTI INDEX_________________________________________________________________________\n",
    df_multi_index.head(),
    "_ SEMICOLON ___________________________________________________________________________\n",
    df_semicolon.head()
)
