...
-- Conocer los datos, su estructura y cómo se recolectaron es fundamental para entender su contenido y posibles problemas. 
Inconsistencias en formatos, datos faltantes, duplicados, y diferencias semánticas entre variables similares son desafios importantes.
...
--Permite a los usuarios controlar y compartir sus datos financieros para acceder a servicios personalizados y competitivos.
...
--Todos permiten compartir datos financieros con consentimiento; benefician al usuario con más opciones, 
transparencia y servicios personalizados.
...
--Valores nulos, duplicados, formatos inconsistentes y distribuciones desbalanceadas. 
Afectan la precisión, generalización y estabilidad del modelo.
...
df_investmentBankCDE = pd.read_csv("data/InvestmentBankCDE.csv")

#lo copiamos para evitar errores de nombre de variable, no los encuento!!! y con esto soluciono el error!!
df_investment = df_investmentBankCDE
...
"""
""" 
El atributo común entre los diferentes conjuntos de datos que permite juntarlos es el "ID". 
Este identificador único se utiliza para vincular la información de un cliente a través de las diferentes empresas
financieras.
"""
#Write your code here
# juntar los dataframes por el atributo común 'customer_id'
# primero sumamos 2 dataframes al dataframe_fusionado y luego el tercero al fusionado
data_frame_fusionado = pd.merge(df_retailbank, df_investmentBankCDE, on='ID', how='outer')
data_frame_fusionado = pd.merge(data_frame_fusionado, df_insurance, on='ID', how='outer')


"""## Pregunta
Indica cuál es la cantidad de registros en cada conjunto de datos.
-- la cantidad de registros en cada conjunto de datos es igual, 1082 registros
 

*¿Qué conclusiones puedes sacar luego de observar los resultados?*
-- Los datos estan alineados por cliente, lo que permite un analisis más completo de los clientes y sus interacciones con las diferentes instituciones financieras.


"""

#Write your code
# recuperamos la cantidad de registros de cada dataframe
num_df_retailbank = len(df_retailbank)
num_df_investBankCDE = len(df_investmentBankCDE)
num_df_insurance = len(df_insurance)

print(f"Cantidad de registros en df_retailbank: {num_df_retailbank}")
print(f"Cantidad de registros en df_investmentBankCDE: {num_df_investBankCDE}")
print(f"Cantidad de registros en df_insurance: {num_df_insurance}")

# conclusiones de los datos. Son muy pocos registros en comparación con los datos de retailbank


"""## Pregunta
¿Has notado algún patrón entre los datos, ya sea entre filas o columnas?ç

-- Si, hay un patrón entre los datos, ya que cada cliente tiene un identificador único (ID) 
que se repite en los diferentes conjuntos de datos. 
Esto permite relacionar la información de un cliente a través de las diferentes instituciones financieras,
 lo que facilita el análisis y la creación de modelos de machine learning para recomendaciones de seguros.
 Tambien los valores True y False en las columnas de seguros e inversiones indican si un cliente tiene o no un 
 producto financiero específico, lo que permite identificar patrones y preferencias entre los clientes.
...
def get_nan_values(data_frame):
    """
    Devuelve un DataFrame con el recuento y porcentaje de valores nulos por columna.
    Maneja errores si el argumento no es un DataFrame válido.
    """
    if data_frame is None or not hasattr(data_frame, "isna"):
        print("Error: El argumento proporcionado no es un DataFrame válido.")
        return pd.DataFrame()  # Devuelve un DataFrame vacío como salida segura
    ...
    return pd.DataFrame({
        'Valores Nulos': nan_count_per_column,
        'Porcentaje (%)': nan_percentage_per_column
...
df_retailbank_nan_values = get_nan_values(df_retailbank)

#Write your code here for df_investment
df_investment_nan_values = get_nan_values(df_investment)

#Write your code here for df_insurance
df_insurance_nan_values = get_nan_values(df_insurance)


"""## Pregunta
*¿Existen valores faltantes en los datos?*
No, los csv estan completos, no hay valores faltantes en los datos.
...
num_duplicates_investment = check_duplicates(df_investmentBankCDE, 'ID')
...
Si, existen datos duplicados en los tres conjuntos de datos. 522 registros en cada uno de los CSV
...
#Write your code here for df_retailbank
estadisticas_retailbank = df_retailbank.describe()

#Write your code here for df_investment
estadisticas_investment = df_investment.describe()

#Write your code here for df_insurance
estadisticas_insurance = df_insurance.describe()



"""### Identificar Valores Únicos:
Ahora, para todas las variables no numéricas, debemos identificar cuántos tipos de datos están registrados 
en cada columna. Implementaremos la función `get_value_counts_non_numeric_columns`, 
la cual obtiene los conteos de valores de las columnas no numéricas en un DataFrame 
y devuelve un diccionario donde las claves son los nombres de las columnas no numéricas 
y los valores son sus respectivos conteos de valores.
"""
...
def get_value_counts_non_numeric_columns(df):
    ...
    # Inicializamos un diccionario para guardar el contador de valores
    # para cada columna no numérica
    value_counts_dict = {}
    # recorremos las columnas no numéricas
    for column in non_numeric_columns:
        # Obtenemos el acumulado de valores únicos en la columna
        value_counts = df[column].value_counts().to_dict()
        # sumamos el contador de valores al diccionario
...
#Write your code here for df_retailbank
dtypes_retailbank = df_retailbank.dtypes

#Write your code here for df_investment
dtypes_investment = df_investment.dtypes

#Write your code here for df_insurance
dtypes_insurance = df_insurance.dtypes


#aqui me he qudado hoy 10/06/2025!!!!!!!!!!! FRANKI

"""## Pregunta
*¿Qué puedes concluir respecto de todas las variables que no son numéricas?*
--son variables categóricas que representan diferentes tipos de productos financieros, como seguros e inversiones.
-- se pueden utilizar para segmentar a los clientes, sus preferencias, operaciones que ralizan con las entidades.
-- son de tipo booleano, tru=tiene ese producto  false=no tiene ese producto


*¿Has identificado algún patrón o característica?*
-- el patron es la codificacion en una variable booleana para saber si un cliente tiene o no un producto contratado
...
- **Gráfico de barras:** Utiliza un gráfico de barras para visualizar la cantidad de datos únicos 
en cada variable no numérica.
- **Gráfico de pastel:** Muestra la distribución de los datos en cada variable no numérica utilizando 
un gráfico de pastel.

### Box plots para columnas numéricas:
- **Box plot para cada columna numérica (excluyendo la columna ID):
** Utiliza box plots para visualizar la distribución de los datos,
 los valores atípicos y la mediana en cada columna numérica.
"""

#Write your code here, add your custom plots  for df_retailbank

# Visualizaciones para variables no numéricas en df_retailbank

#2025/06/13 para corregir un error en el campo Investimento Fundo_cambiais, quitamos los espacios en blanco
#df_retailbank.columns = df_retailbank.columns.str.strip().str.replace(' ', '_')

for col in df_retailbank.select_dtypes(exclude='number').columns:
    # Gráfico de barras
    if col != 'Investimento Fundos_cambiais':
        
       plt.figure(figsize=(8, 4))
       sns.countplot(x=col, data=df_retailbank)
       plt.title(f'RetailBankEFG - Frecuencia de {col}')
       plt.ylabel('Cantidad')
       plt.xlabel(col)
    ...
    df_retailbank[col].value_counts().plot.pie(autopct='%1.1f%%', startangle=90)
    plt.title(f'RetailBankEFG - Distribución de {col}')
    plt.ylabel('')
    plt.tight_layout()
    plt.show()

# Box plots para columnas numéricas en df_retailbank (excluyendo 'customer_id' porque es la clave primaria)
numeric_cols = df_retailbank.select_dtypes(include='number').columns
numeric_cols = [col for col in numeric_cols if 'id' not in col.lower()]

for col in numeric_cols:
    plt.figure(figsize=(8, 4))
    sns.boxplot(x=df_retailbank[col])
    plt.title(f'RetailBankEFG - Boxplot de {col}')
    plt.xlabel(col)
    plt.tight_layout()
    plt.show()

#Write your code here, add your custom plots for df_investment
# Visualizaciones para variables no numéricas en df_investment
# nos embuclamos en el df_investmentBankCDE para leer solo las columnas no numéricas
for col in df_investmentBankCDE.select_dtypes(exclude='number').columns:
    # Gráfico de barras
    if col != 'Investimento Fundos_cambiais':
       plt.figure(figsize=(8, 4))
       sns.countplot(x=col, data=df_investmentBankCDE)
       plt.title(f'InvestmentBANKCDE - Frecuencia de {col}')
       plt.ylabel('Cantidad')
       plt.xlabel(col)
       plt.xticks(rotation=45)
       plt.tight_layout()
    ...
    # Gráfico de pastel
    plt.figure(figsize=(6, 6))
    df_investmentBankCDE[col].value_counts().plot.pie(autopct='%1.1f%%', startangle=90)
    plt.title(f'InvestmentBANKCDE - Distribución de {col}')
    plt.ylabel('')
    plt.tight_layout()
    plt.show()

# Box plots para columnas numéricas en df_retailbank (excluyendo 'customer_id' porque es la clave primaria)
numeric_cols = df_investmentBankCDE.select_dtypes(include='number').columns
numeric_cols = [col for col in numeric_cols if 'id' not in col.lower()]

for col in numeric_cols:
    plt.figure(figsize=(8, 4))
    sns.boxplot(x=df_investmentBankCDE[col])
    plt.title(f'InvestmentBankCDE - Boxplot de {col}')
    plt.xlabel(col)
    plt.tight_layout()
    plt.show()

#Write your code here, add your custom plots for df_insurance
for col in df_insurance.select_dtypes(exclude='number').columns:
    # Gráfico de barras
    if col != 'Investimento Fundos_cambiais':
       plt.figure(figsize=(8, 4))
       sns.countplot(x=col, data=df_insurance)
       plt.title(f'InsuranceCompanyABC - Frecuencia de {col}')
       plt.ylabel('Cantidad')
       plt.xlabel(col)
       plt.xticks(rotation=45)
       plt.tight_layout()
    ...
    # Gráfico de pastel
    plt.figure(figsize=(6, 6))
    df_insurance[col].value_counts().plot.pie(autopct='%1.1f%%', startangle=90)
    plt.title(f'InsuranceCompanyABC - Distribución de {col}')
    plt.ylabel('')
    plt.tight_layout()
    plt.show()

# Box plots para columnas numéricas en df_retailbank (excluyendo 'customer_id' porque es la clave primaria)
numeric_cols = df_insurance.select_dtypes(include='number').columns
numeric_cols = [col for col in numeric_cols if 'id' not in col.lower()]

for col in numeric_cols:
    plt.figure(figsize=(8, 4))
    sns.boxplot(x=df_insurance[col])
    plt.title(f'InsuranceCompanyABC - Boxplot de {col}')
    plt.xlabel(col)
    plt.tight_layout()
    plt.show()

"""## Preguntas
1. *¿Cuál de las dos opciones sugieres utilizar para evaluar datos no numéricos: 
imprimir los valores o crear visualizaciones?*
--visualizaciones porque permiten una mejor comprension de la informacion 

2. *¿Qué otros tipos de visualizaciones se te ocurren que podrías sugerir? Justifica tu respuesta.*
--barras apiladas por cliente, para ver la cantidad de productos que tiene cada cliente,
  mapas de calor para ver las semejanzas entre las variables no numericas,

3. *¿Existe un desbalance en los datos, es decir, existen más tipos que corresponden a una clase?
-- si, creo que hay desbalanceo en los datos, hay muchos clientes sin nada contratado, por ejemplo:
11877.000,"F","F","F","F","F","F","F","F","F", este ID tiene cero productos contratados

¿Cuál es la clase y cómo crees que esto puede afectar al construir modelos de machine learning?*
la clase es la de los clientes que no tienen productos contratados,
Esto puede afectar al construir modelos de machine learning porque puede llevar a un 
modelo sesgado que no generalice bien para los clientes que si tienen productos contratados,
ya que el modelo puede aprender a predecir 
la clase mayoritaria (clientes sin productos) en lugar de las clases minoritarias (clientes con productos).    
...
--La distribución es asimétrica, con valores atípicos visibles, especialmente en ingresos. 
Algunas regiones muestran mayor dispersión, como SE y S, lo que sugiere que los datos son variables entre grupos.

* *¿Existen datos atípicos en el conjunto de datos?*
Si, creo que son atipicos los valores de edad e ingresos. Hay valores alejados de una media.

*¿Cómo podrías corregir estos datos? Justifica tu respuesta*.
Podríamos eliminar los valores atípicos utilizando técnicas como el método 
del rango intercuartílico (IQR) o la desviación estándar.
...
-- Sí, es necesario realizar técnicas de imputación para completar los datos faltantes,
 ya que estos pueden afectar la calidad del análisis y los modelos de machine learning.


2. *¿Debemos realizar tareas de imputación de valores luego de analizar los datos?*
Si, debemos realizar tareas de imputación de valores para asegurar que los 
modelos de machine learning puedan trabajar con datos completos y precisos.

3. *¿Por favor, describe al menos dos técnicas de imputación de datos para valores faltantes basadas en métodos estadísticos?*
Tenemos dos técnicas comunes de imputación de datos para valores faltantes basadas en métodos estadísticos:
1. **Imputación por la Media/Mediana/Moda**: Esta técnica sustituye los valores que faltan con la media,
 mediana o moda de la columna correspondiente. 
se puede usar cuando los datos son numéricos y podemos asumir que la distribución se considera normal. 

2. **Imputación por Regresión**: Esta técnica utiliza un modelo de regresión para predecir
 los valores faltantes basándose en otras variables del conjunto de datos que tenemos.


4. *¿Por favor, describe al menos dos técnicas de imputación de datos para valores faltantes basadas en métodos predictivos?*
Tenemos dos técnicas comunes de imputación de datos para valores faltantes basadas en métodos predictivos:
   tecnica K-Nearest Neighbors: Esta técnica utiliza los datos mas cercanos para asignar los que faltan
    a un valor que se considera adecuado.

    Se basa en la idea de que los puntos de datos cercanos entre sí son similares y, por lo tanto, se puede utilizar la información de los vecinos más cercanos para predecir el valor faltante.
   tecnica de Árboles de Decisión: Esta técnica utiliza árboles de decisión para predecir 
   los valores faltantes basándose en otras variables del conjunto de datos. 
   Hace un recalculo con lo que tenemos.
...
df_investmentBankCDE.drop_duplicates(inplace=True)  
...
-- Es importante para evitar sesgos en el análisis. Los duplicados pueden distorsionar los resultados,
    pueden hacer que algunos valores predominen, y que hagan que se altere el resultado en su favor.
...
Como pudiste observar en las gráficas anteriores, existen diferentes valores atípicos que se encuentran fuera del rango. Para abordar esto, vamos a crear una opción que nos permita excluir los datos atípicos de nuestro conjunto de datos `df_insurance`. 
Para tomar esta decisión, eliminaremos todos los registros que sean menores al primer cuartil 
y todos aquellos mayores al tercer cuartil. El resultado final se asignará al DataFrame `df_insurance`.
 Para llevar a cabo este proceso, haremos uso de la clase `OutlierRemover`.
"""
...
# Franki 13/06/2025 (hacia arriva esta listo y se ejecuta bien)
...
#Write your code here
num_registros_eliminando_atipicos = len(df_insurance)
print(f"Número de registros en df_insurance después de eliminar datos atípicos: {num_registros_eliminando_atipicos}")

"""## Pregunta
*Explica con tus propias palabras cómo podría afectar una diferencia significativa en el tamaño 
del conjunto de datos antes y después de eliminar los valores atípicos.
 ¿Qué implicaciones podría tener esto en los resultados de un modelo de machine learning?*

 Eliminar datos atipicos reduce el volumen de datos, puede mejorar la precision y calidad pero tambien
 puede llevar a perder informacion que nos resulte valiosa.
...
*Graficar la región(Regiao) en función de la edad(Idade), del conjunto de datos `df_insurance`, 
utilizando la función `plot_boxplot_violinplot`.*
"""
...
print("-------Fin delos graficos de boxplot y violinplot")

"""## Pregunta
*¿Cómo crees que la eliminación de datos atípicos ha afectado la distribución 
y los patrones observados en las gráficas? 
Los graficos no estan tan disperdos, parecen mas concentrados en edades e ingresos.

¿Qué cambios específicos puedes identificar en los datos después de esta eliminación?*
Las mdedias se han acercado. Los maximos y minimos no estan tan lejanos de la media.
...
Luego, para ajustar la escala, vamos a utilizar las clases StandardScaler y ColumnTransformer. 
Debes implementar el código necesario para realizar la conversión.

Para la clase DataScaleImputer, debes investigar un poco cómo realizar la conversión 
a una escala estándar (StandardScaler) mediante el uso de la clase ColumnTransformer.
 Puedes revisar la documentación oficial de sklearn.
...
class DataScaleImputer(BaseEstimator, TransformerMixin):
    ...
    # Verificación de columnas válidas
        if self.columns is None:
           raise ValueError("Debe especificar columnas para escalar/imputar.")
      ...
      # Asegurar que las columnas existen
        ...
        # Solo aplicar el pipeline a las columnas deseadas
        data = X[self.columns]
    ...
    # Construir DataFrame con mismas columnas transformadas
        X_imputed_df = pd.DataFrame(data=X_transform, columns=self.columns, index=X.index)

    # Reemplazar solo las columnas afectadas en el DataFrame original
        X[self.columns] = X_imputed_df[self.columns]
        ...
        #transformamos las columnas especificadas con StandardScaler
        #con el objetivo de estandarizar los datos
        transformer = ColumnTransformer(
            transformers=[
                ('scale', self.scaler, self.columns)  
            ],      
        ...
        return data.dropna()  # Return the transformed 
        '''

"""*Ejecuta la transformación utilizando la clase `DataScaleImputer` y asigna el resultado a `df_insurance`*"""

# Write you code here
print("----Transformacion de df_insurance usando clade DataScaleImputer")
...
# Write you code here
print("----Estadísticas básicas del conjunto de datos df_insurance:")
...
# Write you code here
#fusionamos los dataframes en base a la columna 'ID' en el dataframe data_frame_merged
print("----Fusion de dataframes por ID")
print("----Fusion de dataframes df_insurance  y df_retailbank por ID")
data_frame_merged = pd.merge(df_insurance, df_retailbank, on='ID', how='outer')

print("----Fusion de dataframes merged df_investmentBankCDE por ID")
data_frame_merged = pd.merge(data_frame_merged, df_investmentBankCDE, on='ID', how='outer')
...
# Write you code here
print("----Cantidad total de registros después de realizar el merge entre los conjuntos de datos:")
print(len(data_frame_merged))

"""*Observamos una visión estadística rápida de los datos mediante la función `describe`.*"""

# Write you code here
print("----Visión estadística rápida de los datos:")
print(data_frame_merged.describe())

"""*Verifica si hay datos faltantes en el DataFrame resultante.*"""

# Write you code here
print("----Verificación de datos faltantes en el DataFrame resultante:")
print(data_frame_merged.isnull().sum())

"""# Correcion nombres columnas

Como has notado, se presentan ciertos inconvenientes en los nombres de las columnas.
 A continuación, intentaremos resolver estos errores identificando 
 y corrigiendo espacios adicionales u otros problemas.
"""

print("----Corrección de nombres de columnas en data_frame_merged")
...
print("---FIN DE CORRECCION DE DATOS Y TRATAMIENTO DE DATOS PARA MODELOS DE MACHINE LEARNING---")
      
#FRANKI 14/06/2025  13:11  HASTA ARRIVA FUNCIONA Y SE EJECUTA MAS O MENOS BIEN

"""## Finalización tramiento de datos

### Tratamiento de datos para modelos de Machine Learning

Como último paso, es necesario ejecutar el siguiente tratamiento a los datos 
con el objetivo de prepararlos para nuestros modelos de Machine Learning. Seguiremos los siguientes pasos:

1. **Creación de la variable a predecir:** 
Se creará una nueva columna llamada "tipo_financiamiento", que será la variable a predecir 
por nuestros modelos de Machine Learning. Esta columna representará el tipo de financiamiento,
 permitiendo identificar si corresponde a una casa, un carro, ambos o ninguno. 
 El siguiente proceso se ejecutará sobre el `data_frame_merged`, generando un nuevo DataFrame
   llamado `data_frame_tipo_financiamiento` con la columna adicional "tipo_financiamiento".

2. **Etiquetado de columnas categóricas:** 
Se etiquetarán las columnas categóricas como multi label.
Las columnas identificadas son "AGE_RANGE", "INCOME_RANGE", "tipo_financiamiento" y "Regiao".

3. **Eliminación de la columna "ID":** Se eliminará la columna utilizada como identificador.

4. **Conversión de valores binarios:** 
Se convertirán todas las columnas con valores 'F' o 'T' a tipos de datos numéricos 0 y 1, respectivamente.
"""
...
#elimino datos faltantes, sino me da error en el codigo sns.countplot
data_frame_tipo_financiamiento.dropna(inplace=True)
...
print("----Estadísticas básicas del conjunto de datos data_frame_tipo_financiamiento:")
print(data_frame_tipo_financiamiento.describe())

"""## Cierre tratamiento de datos
Es crucial comprender que el tratamiento de datos no es solo una etapa preliminar,
sino un proceso continuo que puede influir significativamente en el rendimiento 
y la precisión de los modelos de Machine Learning. 
Al abordar de manera efectiva problemas como valores faltantes,
 valores atípicos y errores de formato, estamos creando un conjunto de datos robusto y confiable,
   lo que a su vez potencia la capacidad predictiva de nuestros modelos.

Hasta este punto, hemos completado varios pasos relacionados con el tratamiento y la limpieza de datos.
 Ahora vamos a continuar con el desarrollo de los diferentes algoritmos de Machine Learning.
...
# Selección de modelo:
Identifica el tipo de problema de aprendizaje (clasificación, regresión, agrupamiento, etc.)
 y selecciona los modelos más adecuados para tu problema.
Experimenta con diferentes algoritmos de machine learning y ajusta sus hiperparámetros para encontrar
 la mejor combinación.
1. Entrenamiento de modelos:
Entrena por lo menos 3 modelos utilizando los datos de entrenamiento. 
Ajusta los parámetros del modelo utilizando algoritmos de optimización como la descenso del gradiente,
 búsqueda de cuadrícula, o búsqueda aleatoria.
Valida el modelo utilizando los datos de validación y ajusta los parámetros según sea necesario 
...
## Pregunta
*¿Cuál es el tipo de problema que estás enfrentando: clasificación o regresión?
es clasifciacion, para predecir mejor los clientes y su financiamiento

 Imprime o grafica el conteo de valores que corresponde a la columna `data_frame_tipo_financiamiento`.*
"""

# Write you code 

# contamos los valores de la columna 'tipo_financiamiento'
count_tipo_financiamiento = data_frame_tipo_financiamiento['tipo_financiamiento'].value_counts()

# Write you code here, add your custom plot
print("----Conteo de valores en la columna 'tipo_financiamiento':")
print(count_tipo_financiamiento)

# pintamos el grafico de barras, ojo!!! a ejecutar deben salir 4 barras!!
plt.figure(figsize=(10, 6))

# corregido tras warning de sns.countplot
sns.countplot(
    data=data_frame_tipo_financiamiento,
    x='tipo_financiamiento',
    hue='tipo_financiamiento', 
    palette='viridis',
    legend=False                
)

plt.title('Conteo de Valores en la Columna tipo_financiamiento')    
...
hasta eeste punto del codigo solo hemos usado el modelo de regresion logistica. Asi que en este punto del codigo
no puedo comparar con otro modelo.

* *¿Logran los modelos etiquetar todas las clases de forma precisa? 
¿Qué estrategias podrían aplicarse para mejorar este aspecto?*

## Extracción de características y Análisis de Componentes Principales(PCA)

Aquí está la corrección:

Ahora vamos a desarrollar validaciones para ver cuáles características son más relevantes para el modelo. 
Para esto, debes a implementar una función llamada  `plot_correlations` que te permita graficar 
las correlaciones del DataFrame `data_frame_tipo_financiamiento`.
"""

def plot_correlations(df_temp):
    #Write your code here
    matriz_correlacion = df_temp.corr()
    ...
    plt.title("Matriz de correlación - data_frame_tipo_financiamiento", fontsize=16)
    plt.xticks(rotation=45)
...
print("----Gráfico de correlaciones del DataFrame data_frame_tipo_financiamiento")
plot_correlations(data_frame_tipo_financiamiento)

"""## Pregunta
* *¿Puedes identificar cuáles columnas son más relevantes y por qué?*
Financiamiento Carro y casa parecen las mas relevantes,
Si alguien tiene fincnacimiento, lo habitual es tener carro o casa, son las mas fercuentes.
...
"""*Utiliza la función `get_most_important_features` e imprime las primeras 6 columnas más relevantes 
del conjunto de datos que están relacionadas con la variable a predecir.*

"""

#Write your code here
matriz_correlacion = data_frame_tipo_financiamiento.corr()
top_seis = get_most_important_features(matriz_correlacion, 'tipo_financiamiento', n=6)
print("----Las 6 columnas más relevantes relacionadas con la variable a predecir 'tipo_financiamiento':")
print(top_seis)


#franki 15/06/2025  noche
...
El PCA esunatécnica matemática que nos permite transformar nuestros datos en un nuevo
 conjunto de variables, conocidas como componentes principales.
 Esto puede ayudarnos a simplificar nuestros datos y a mejorar la eficiencia computacional de nuestros
 modelos

*Modifica la función `create_pca_model`. 
Los parámetros de entrada son el conjunto de datos sin la variable a predecir. 
Se creará un modelo de Análisis de Componentes Principales (PCA), el cual tendrá como parámetro 
el número N de componentes a identificar. 
El resultado será el modelo exportado y la transformación hacia las componentes principales
 luego de evaluar el modelo.*

* Esta función toma como entrada `X_train`, que representa las características del conjunto de entrenamiento 
sin la variable objetivo, y `n_components`, que indica el número de componentes principales que se desea conservar.

* Dentro de la función, se instancia un objeto PCA con el número de componentes especificado. 
Luego, se ajusta este objeto PCA a las características del conjunto de entrenamiento para determinar 
las características transformadas.
...
def create_pca_model(X_train, n_components):
    ...
    # varianza
    print("Varianza ratio:", pca_model.explained_variance_ratio_)
    # varianza acumulada
    print("Varianza acumulada:", pca_model.explained_variance_ratio_.cumsum())
    # shape
    print("Rasgos transformados:", X_principal.shape)
    # head de los 5 primeros registros
...
"""Llamamos a la función `create_pca_model`, pasando como argumentos el DataFrame `data_frame_tipo_financiamiento`
 y `n_components` igual a 10, para determinar las 10 componentes principales del conjunto de datos.
  Luego, graficamos la varianza acumulada."""
...
"""*Implementa un gráfico tipo pie que muestre cómo lucen los datos después de realizar 
el tratamiento para abordar el desbalance.*"""

#Write your code 


#Cuenta cuántas veces aparece cada clase en y_reshaped 
# (que contiene la variable tipo_financiamiento transformada a números).
conteo_tipo_financiamiento_label = y_reshaped.value_counts().rename(index=tipo_financiamiento_mapping)
#titulo del grafico
conteo_tipo_financiamiento_label.plot.pie(title='Distribución de Tipo de Financiamiento Después del Tratamiento')
# 
#y_reshaped.value_counts().plot.pie(title='Distribución de Tipo de Financiamiento Después del Tratamiento')



X_reshaped = pd.DataFrame(X_reshaped).reset_index(drop=True)
y_reshaped = pd.Series(y_reshaped).reset_index(drop=True)

# vamos a mapear los nombres para que se vean en el grafico en lugar de los numeros
y_nombres = y_reshaped.map(tipo_financiamiento_mapping)

#grafico de barras
plt.figure(figsize=(8, 6))
sns.countplot(x=y_nombres, palette='viridis', order=sorted(y_nombres.unique()))
plt.title('Distribución de Tipo de Financiamiento Después del Tratamiento')
plt.xlabel('Tipo de Financiamiento')
plt.ylabel('Conteo')
plt.xticks(rotation=45)
plt.show()
...
"""*Separa los datos en conjuntos de entrenamiento y test utilizando la función `startified_train_test_split()`.
 Luego, implementa un modelo que haga uso del siguiente clasificador. 
 Puedes probar modificando los hiperparámetros y evaluar los resultados. 
 También puedes optar por modificar los parámetros de las clases `RandomUnderSampler` y
   `SMOTE` del paso anterior.*
...
#aseguramos que tienen la misma longitud, sino mas abajo peta
#X_reshaped = pd.DataFrame(X_reshaped).reset_index(drop=True)
#y_reshaped = pd.Series(y_reshaped).reset_index(drop=True)
...
Con mis conocimientos actuales, me cuesta evaluar la eficiencia de los modelos, pero segun los manuales de scikit-learn,
 el GradientBoostingClassifier es un modelo muy eficiente para problemas de clasificación,
   ya que combina múltiples árboles de decisión para mejorar la precisión y reducir el sobreajuste.
 Se comenta que tiene gran capacidad para manejar datos desbalanceados, por lo que para los datos que manejamos aqui
 parece el model mas adecuado.
...
Com dije antes me es dificil evaluar con mis conocimientos actuales cual es el mejor modelo.
Para estos datos desbalanceados como los que tenemos en los csv, se supone que el GradientBoostingClassifier es el mejor modelo.


* *Investiga qué otras opciones pueden ser utilizadas para enfrentar el problema de datos desbalanceados 
e implementa un ejemplo.*
He encontrado lo siguiente: 
se puede utilizar ADASYN, que genera ejemplos sintéticos enfocados en áreas difíciles para el modelo.
 Otra opción es usar el parámetro class_weight='balanced' en clasificadores como RandomForest o LogisticRegression, 
lo que penaliza los errores cometidos sobre clases minoritarias.


* *Investiga qué son los modelos de ensamble e implementa un corto ejemplo.*

Los modelos de ensamble combinan múltiples modelos (débilmente o fuertemente aprendidos)
 para mejorar la precisión y robustez. Dos ejemplos comunes son:
VotingClassifier: combina varios modelos y predice por mayoría de votos.
Bagging y Boosting: como Random Forest y Gradient Boosting.

Un ejemplo simple con VotingClassifier podría ser:
...
], voting='soft')  # o 'hard' para mayoría simple
